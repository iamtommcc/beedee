...
export async function GET() {
  ...
  try {
    const webpagesToScrape = await sql<Pick<WebpageConfig, "id" | "url">[]>`
    SELECT id, url FROM webpages_to_scrape
  `

    if (webpagesToScrape.count === 0) {
      console.log("[Cron] No webpages configured for scraping.")
      return NextResponse.json({ message: "No webpages configured." })
    }

    console.log(`[Cron] Found ${webpagesToScrape.count} webpages to scrape.`)
    ...
    const scrapingPromises = webpagesToScrape.map((page) =>
    ...
    return NextResponse.json({ message: `Scraping initiated for ${webpagesToScrape.count} URLs.` })
  } catch (error: any) {
    console.error("[Cron] Error fetching webpages for scraping:", error)
    return NextResponse.json({ error: `Failed to start daily scrape: ${error.message}` }, { status: 500 })
  }
}
